...
const title = computed(() => {
  if (props.new) return 'Create Shape';
  if (props.edit) return 'Edit Shape';
  if (props.readonly) return 'View Shape'; 
  return 'Shape Form';
});

const subtitle = computed(() => {
  if (props.new) return 'Fill out the information below to create a shape';
  if (props.edit) return 'Use the form below to edit the shape';
  if (props.readonly) return 'View Shape Information';
  return '';
});
...
const fillShapeInformation = () => {
  if (props.edit || props.readonly) {
    name.value.value = props.shape?.name || '';
    slug.value.value = props.shape?.slug || '';
  }
}

onMounted(() => {
  fillShapeInformation();
});
watch(
  () => props.shape,
  () => {
...
const handleCreate = async (values: Shape) => {
  try {
    ...
    if (shape.length) router.push(`/shapes/${shape[0].id}`);
    notify({
    ...
    console.error(e);
    notify({
      title: 'Error creating shape',
      text: e?.message || 'An error occurred trying to create a shape. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
...
const handleUpdate = async (values: Shape) => {
  try {
    isLoading.value = true;
    const form = JSON.parse(JSON.stringify(values));
    if (!form.slug) form.slug = slugify(form.name);
    const { data: shape, error } = await supabase
      .from('shape')
      .update(form)
      .eq('id', props?.shape?.id || 0)
      .select();
    if (error) throw error;
    if(shape.length) {
      name.value.value = shape[0].name;
      slug.value.value = shape[0].slug;
    }
    notify({
    ...
    notify({
      title: 'Error updating shape',
      text: e?.message || 'An error ocurred trying to update the shape. Please contact TOP support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
