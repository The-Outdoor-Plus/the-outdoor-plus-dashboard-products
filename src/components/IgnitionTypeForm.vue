  ...
  <div class="tw-w-full">
    <v-card class="py-12 px-10" :loading="isLoading">
      <v-btn
        ...
        icon="mdi-arrow-left"
        flat
        @click="$router.push('/ignition-types')"
      ></v-btn>
        ...
        <div class="tw-w-full tw-flex tw-flex-col lg:tw-flex-row">
          ...
          <div class="tw-w-full tw-mt-3 lg:tw-mt-0 lg:tw-w-7/12 xl:tw-w-4/12">
            <v-text-field
              ...
              density="compact"
              name="Slug"
              placeholder="ignition-type-slug"
...
interface Props {
  ...
  edit?: boolean;
  readonly?: boolean;
  ignitionType?: IgnitionType | null;
  loading?: boolean;
}
...
const title = computed(() => {
  if (props.new) return 'Create Ignition Type';
  if (props.edit) return 'Edit Ignition Type';
  if (props.readonly) return 'View Ignition Type';
  return 'Ignition Type Form';
});

const subtitle = computed(() => {
  if (props.new) return 'Fill out the information below to create a ignition type';
  if (props.edit) return 'Use the form below to edit the ignition type';
  if (props.readonly) return 'View Ignition Type Information';
  return '';
});
...
const fillIgnitionTypeInformation = () => {
  if (props.edit || props.readonly) {
    name.value.value = props.ignitionType?.name || '';
    slug.value.value = props.ignitionType?.slug || '';
    skuCode.value.value = props.ignitionType?.sku_code || '';
  }
}

onMounted(() => {
  fillIgnitionTypeInformation();
});
watch(
  () => props.ignitionType,
  () => {
...
const handleCreate = async (values: IgnitionType) => {
  try {
    isLoading.value = true;
    const form = JSON.parse(JSON.stringify(values));
    if (!form.slug) form.slug = slugify(form.name);
    else form.slug = slugify(form.slug);
    const { data: ignition , error } = await supabase
      .from('ignition')
      .insert(form)
      .select();
    if (error) throw error;
    if (ignition.length) {
      await createAttributeValue('Ignition Type', undefined, 'ignition', ignition[0].id);
      router.push(`/ignition-types/${ignition[0].id}`);
    }
    notify({
      title: 'Ignition Type created successfully',
      type: 'success',
      duration: 6000,
    });
  } catch (e: any) {
    console.error(e);
    notify({
      title: 'Error creating ignition type',
      text: e?.message || 'An error occurred trying to create a ignition type. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
...
const handleUpdate = async (values: IgnitionType) => {
  try {
    isLoading.value = true;
    const form = JSON.parse(JSON.stringify(values));
    if (!form.slug) form.slug = slugify(form.name);
    ...
    const { data: ignition, error } = await supabase
      .from('ignition')
      .update(form)
      .eq('id', props?.ignitionType?.id || 0)
      .select();
    if (error) throw error;
    if(ignition.length) {
      name.value.value = ignition[0].name;
      slug.value.value = ignition[0].slug;
      skuCode.value.value = ignition[0].sku_code;
    }
    notify({
      title: 'Ignition Type updated successfully',
      type: 'success',
      duration: 6000,
    });
  } catch (e: any) {
    ...
    notify({
      title: 'Error updating ignition type',
      text: e?.message || 'An error ocurred trying to update the ignition type. Please contact TOP support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
