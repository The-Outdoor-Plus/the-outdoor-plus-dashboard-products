    ...
    >
      <template v-slot:top>
        <v-toolbar flat compact color="blue-darken-2">
          <v-toolbar-title>Gas Types</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-btn color="white" dark to="/gas-types/new" exact>New Gas Type <v-icon icon="mdi-plus" class="ml-2"></v-icon></v-btn>
        </v-toolbar>
        <v-dialog v-model="dialogDelete" max-width="600px">
          <v-card class="pt-4 pb-3" :loading="deleteLoading">
            <v-card-title class="text-h5">Are you sure you want to delelete this gas type?</v-card-title>
            <v-card-text>
              Gas type <strong>{{ itemToDelete?.name }}</strong> will be deleted.
      ...
      <template v-slot:item.actions="{ item }">
        <v-btn
          ...
          icon="mdi-eye"
          variant="text"
          :to="`/gas-types/${item.raw.id}`"
        ></v-btn>
        <v-btn
          size="small"
          icon="mdi-pencil"
          variant="text"
          :to="`/gas-types/edit/${item.raw.id}`"
        ></v-btn>
        <v-btn
          size="small"
...
const deleteItemConfirm = async () => {
  try {
    ...
    const { error } = await supabase.from('gas').delete()
      .eq('id', itemToDelete?.value?.id);
    if (error) throw error;
    notify({
      type: 'success',
      title: 'Gas Type deleted successfully',
      duration: 6000,
    });
    ...
    console.error(e);
    notify({
      title: 'Error deleting gas type.',
      text: e?.message || 'An error ocurred trying to delete gas type. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
...
const loadItems = async ({ page, itemsPerPage, sortBy }: TableOptions) => {
  try {
    ...
    if (searchFilter.value) {
      const { data: gasTypes, error } = await supabase
        .rpc('search_gas_types', {
      ...
      if (error) throw error;
      data.serverItems = gasTypes;
      totalItems.value = gasTypes?.[0]?.count || 0;
    } else {
      const { data: gasTypes, error, count } = await supabase
        .from('gas')
        .select('*', { count: 'exact' })
    ...
    console.error(e);
    notify({
      title: 'Error loading gas types.',
      text: e?.message || 'An error ocurred trying to load gas types. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
