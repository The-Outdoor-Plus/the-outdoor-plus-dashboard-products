<template>
  <div class="tw-w-full">
    <div v-if="parentName" class="tw-text-2xl tw-font-semibold tw-mb-4">
      {{ parentName }} - Variations
      <div class="tw-mb-1.5">
        <RouterLink
          class="tw-text-base tw-font-normal tw-text-blue-700 tw-transition-all hover:tw-underline"
    ...
    <div class="tw-mt-8 tw-mb-16 tw-w-full">
      <div v-if="isAdmin" class="tw-w-full tw-flex tw-justify-end">
        <v-btn
          color="light-blue-lighten-5"
          class="tw-mb-3"
        >
          Show Pricing: {{ pricingToShow.name }}
          <v-menu activator="parent">
            <v-list>
              <v-list-item
                v-for="(item) in typePricingList"
                :key="item.id"
                :value="item.name"
              >
                <v-list-item-title @click="pricingToShow = item">{{ item.name }}</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-menu>
        </v-btn>
              ...
              <template v-slot:default>
                <div class="tw-ml-8 tw-flex tw-justify-between tw-flex-wrap tw-items-center">
                  <div>
                    ...
                    <div v-if="item.parent_id && !isAdmin" :class="{ 'tw-text-green-600': !!item.price }">
                      {{ formatPrice(item?.price || null) }}
                    </div>
                    <template
                      v-if="item.parent_id && isAdmin"
                    >
                      <template v-if="pricingToShow.id === 'retail'">
                        <div :class="{ 'tw-text-blue-600': !!item?.price?.map_price }">
                          MAP: {{ formatPrice(item?.price?.map_price || null) }}
                        </div>
                        <div :class="{ 'tw-text-green-600': !!item?.price?.msrp_price }">
                          MSRP: {{ formatPrice(item?.price?.msrp_price || null) }}
                        </div>
                      </template>
                      <template v-else>
                        <div :class="{ 'tw-text-green-600': !!item?.price?.[pricingToShow.id] }">
                          {{ pricingToShow.name }}: {{ formatPrice(item?.price?.[pricingToShow.id] || null) }}
                        </div>
                      </template>
                    </template>
                  </div>
...
const pricingToShow = ref({ id: 'retail', name: 'Retail' });

const typePricingList = ref([
  { id: 'dealer_price', name: 'Dealer' },
  { id: 'distributor_price', name: 'Distributor' },
  { id: 'group_price', name: 'Group' },
  { id: 'internet_price', name: 'Internet' },
  { id: 'landscape_price', name: 'Landscape' },
  { id: 'master_distributor_price', name: 'Master Distributor' },
...
const parentName = computed(() => {
  return route?.query?.['parent-name'];
})
...
const onEnterSearch = async () => {
  await loadProductsList();
  let baseURL = `/search-variations`;
  if (route.params.parentid) {
    baseURL = `${baseURL}/${route.params.parentid}`
  }
  if (search.value) {
    router.push(`${baseURL}?s=${search.value}&page=${page.value}`);
  } else {
    router.push(baseURL);
  }
}
...
const getPricing = async (productId: number, isVariation: boolean) => {
  try {
    ...
    let currentRolePricing = `${(userStore.user?.user_metadata.role as string).toLowerCase()}_price`;
    if (isAdmin.value) {
      currentRolePricing = `dealer_price, distributor_price, group_price, internet_price, landscape_price, master_distributor_price, map_price, msrp_price`;
    }
    if ((availablePrices.includes(currentRolePricing) || isAdmin.value) && isVariation) {
      const { data, error } = await supabase
        .from('variation')
