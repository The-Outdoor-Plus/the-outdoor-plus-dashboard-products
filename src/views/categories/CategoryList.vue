    ...
    >
      <template v-slot:top>
        <v-toolbar flat compact color="blue-darken-2">
          ...
          <v-spacer></v-spacer>
          <v-btn color="white" dark to="/categories/new" exact>New Category<v-icon icon="mdi-plus" class="ml-2"></v-icon></v-btn>
        </v-toolbar>
...
const loadItems = async ({ page, itemsPerPage, sortBy }: TableOptions) => {
  try {
    ...
    if (searchFilter.value) {
      ...
      totalItems.value = categories?.[0]?.count || 0;
    } else {
