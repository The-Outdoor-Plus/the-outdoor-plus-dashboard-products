    ...
    >
      <template v-slot:top>
        <v-toolbar flat compact color="blue-darken-2">
          ...
          <v-spacer></v-spacer>
          <v-btn color="white" dark to="/materials/new" exact>New Materials<v-icon icon="mdi-plus" class="ml-2"></v-icon></v-btn>
        </v-toolbar>
        <v-dialog v-model="dialogDelete" max-width="600px">
          <v-card class="pt-4 pb-3" :loading="deleteLoading">
            <v-card-title class="text-h5">Are you sure you want to delelete this material?</v-card-title>
            <v-card-text>
              Material <strong>{{ itemToDelete?.name }}</strong> will be deleted.
      ...
      <template v-slot:item.actions="{ item }">
        <v-btn
          ...
          icon="mdi-eye"
          variant="text"
          :to="`/materials/${item.raw.id}`"
        ></v-btn>
        <v-btn
          size="small"
          icon="mdi-pencil"
          variant="text"
          :to="`/materials/edit/${item.raw.id}`"
        ></v-btn>
        <v-btn
          size="small"
...
const deleteItemConfirm = async () => {
  try {
    ...
    const { error } = await supabase.from('material').delete()
    ...
    notify({
      ...
      text: e?.message || 'An error ocurred trying to delete material. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
...
const loadItems = async ({ page, itemsPerPage, sortBy }: TableOptions) => {
  try {
    ...
    if (searchFilter.value) {
      ...
      if (error) throw error;
      data.serverItems = materials;
      totalItems.value = materials?.[0]?.count || 0;
    } else {
    ...
    notify({
      ...
      text: e?.message || 'An error ocurred trying to load materials. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
