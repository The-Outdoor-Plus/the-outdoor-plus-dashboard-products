    ...
    >
      <template v-slot:top>
        <v-toolbar flat compact color="blue-darken-2">
          <v-toolbar-title>Shapes</v-toolbar-title>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <v-btn color="white" dark to="/shapes/new" exact>New Shape<v-icon icon="mdi-plus" class="ml-2"></v-icon></v-btn>
        </v-toolbar>
        <v-dialog v-model="dialogDelete" max-width="600px">
          <v-card class="pt-4 pb-3" :loading="deleteLoading">
            <v-card-title class="text-h5">Are you sure you want to delelete this shape?</v-card-title>
            <v-card-text>
              Shape <strong>{{ itemToDelete?.name }}</strong> will be deleted.
      ...
      <template v-slot:item.actions="{ item }">
        <v-btn
          ...
          icon="mdi-eye"
          variant="text"
          :to="`/shapes/${item.raw.id}`"
        ></v-btn>
        <v-btn
          size="small"
          icon="mdi-pencil"
          variant="text"
          :to="`/shapes/edit/${item.raw.id}`"
        ></v-btn>
        <v-btn
          size="small"
...
const deleteItemConfirm = async () => {
  try {
    ...
    const { error } = await supabase.from('shape').delete()
    ...
    console.error(e);
    notify({
      title: 'Error deleting shape.',
      text: e?.message || 'An error ocurred trying to delete shape. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
...
const loadItems = async ({ page, itemsPerPage, sortBy }: TableOptions) => {
  try {
    ...
    if (searchFilter.value) {
      ...
      if (error) throw error;
      data.serverItems = shapes;
      totalItems.value = shapes?.[0]?.count || 0;
    } else {
      const { data: shapes, error, count } = await supabase
        .from('shape')
        .select('*', { count: 'exact' })
    ...
    notify({
      ...
      text: e?.message || 'An error ocurred trying to load shapes. Please contact TOP Support.',
      type: 'error',
      duration: 6000,
    });
  } finally {
